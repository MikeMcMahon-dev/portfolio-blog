---
title: "Failure Detection Dashboard — Observability for AI Agent Failures"
date: 2026-04-04
slug: failure-detection-dashboard
status: published
---
...
4. Look at **Runbooks** (`docs/runbooks/`) for diagnosis steps

**When Something Fails Unexpectedly:**

- Navigate to **Failure Mode Breakdown** — which scenario failed?
- Read corresponding runbook: e.g., `cascading-failure.md` if fail-complex has high retry count
...
- Push metrics via Pushgateway (env: `PUSHGATEWAY_URL=http://192.168.100.54:9091`)
- Run eval harness: `python -m multi_agent_lab.main eval --scenarios taskfiles/eval-scenarios.yaml`
- Metrics appear in Prometheus within 15s
- Dashboard refreshes within 30s

## Infrastructure Setup

**Monitoring stack** (Prometheus + Pushgateway + Grafana) lives in `monitoring/` directory:
...
│   └── dashboards/            — Dashboard JSONs (provisioned as code)
└── README.md                  — Setup + maintenance
```
...
- Dashboard: `monitoring/grafana/provisioning/dashboards/failure-detection-dashboard.json`
- Runbooks: `multi-agent-lab/docs/runbooks/` (6 guides)
- ADR-003: `agent-lab/docs/decisions/ADR-003-prometheus-pushgateway.md` (metrics design)
- Eval scenarios: `multi-agent-lab/taskfiles/eval-scenarios.yaml` (8 scenarios)
